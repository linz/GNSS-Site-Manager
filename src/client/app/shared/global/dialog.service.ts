...
export class DialogService {
  ...
  /*
   * Opens a customised dialog asking user whether to reload a page without saving changes made
   * Very similar to close but using a separate function for cleaner code
   */
  public confirmRevertDialog(msg: string, okCallback: () => any, cancelCallback: () => any) {
    let header: string = '<div class="title">Confirm Reload Page</div>';
    let body: string = '<div class="body"><p/><p>' + msg + '</p><p/></div>';
    let footer: string = '<p class="footer">Do you really want to reload the page?</p>';
    let msgHtml: string = '<div>' + header + body + footer + '</div>';
    this.showConfirmDialog(msgHtml, okCallback, cancelCallback);
  }
